...
| sodium hydroxide | NaOH | fixed compound; production route remains separate |
| potassium hydroxide | KOH | fixed compound; production route remains separate |
| barium nitrate | Ba(NO3)2 | fixed compound |
| graphite | C | allotrope identity retained by production policy |
| pyrolytic carbon | C | chemistry known; production process is separate |
| hard carbon | C | chemistry known; production process is separate |

`DIRECT` means decomposition is chemically representable. It does not mean generic reconstruction is the correct industrial production route.
...
| zirconium-molybdenum | Zr + Mo family | `PROCESS`; grade/process matter |
...
NuclearCraft 1.2.34 is treated according to the states it actually models. Matterworks does not add a fictional UF6 chain merely because real enrichment systems commonly use uranium hexafluoride.
...
Ordinary NuclearCraft U/Th/Po/Ra forms do not receive generic Dissolver routes before the late transmutation programme. This prevents radioactive feed items from being flattened into ChemLib units and reconstructed around enrichment/irradiation progression.
